...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        /* 필기.
        *   다차원 배열
        *    다차원 배열은 2차원 이상의 배열을 의미한다.(일반적으로는 사람의 인지 범위에 맞게 최대 3차원
        * */
...
//        }
        ...
        /* 필기. 관리된 1차원 배열이 없기에 nullPointerException 에러가 발생하는 경우 */
//        for (int i = 0; i < iArr2.length; i++) {            // 1차원 배열을 고르는 for문
//            for (int j = 0; j < iArr2[i].length; j++) {     // 관리된 1차원 배열이 없기에 nullPointerException 에러
//                iArr2[i][j] = num++;
//            }
//            System.out.println(Arrays.toString(iArr2[i]));
//        }
        
        /* 필기. 가변 배열은 아직 1차원 배열들이 생성되어 있지 않다. (NullPointerException도 발생 가능)
        *       따라서, length라는 변수를 이용해 반복문안에서 1차원 배열을 동시에 선언한다.
        * */
        int length = 1;
        for (int i = 0; i < iArr2.length; i++) {            // 1차원 배열을 고르는 for문
