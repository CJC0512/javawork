...
public class Application {
    public static void main(String[] args) {
        ...
        try {
             ...
             *       (단, 범위는 너무 크지 않게 적당히 지정할 것(관련없는 것만))
             * */
            et.checkEnoutghMoney(10000, 50000);
            et.checkEnoutghMoney(100000, 50000);
            System.out.println("이전에 문제 없었으면 실행 될 try의 나머지 부분");     // 예외가 발생하여 catch로 이동되어 해당 행이 작동하지 않는다.
        } catch (Exception e) {

            /* 설명.
             *   try 구문에서 발생한 예외를 처리하기 위한 구문으로, 우리가 원하는 방식으로 예외 상황에 대해 처리할 수 있다.
             *   (단순 출력 또는 예외 타입에서 제공하는 메소드를 활용한 처리 모두 가능)
             *   catch 구문에 아무런 처리 구문도 작성하지 않으면 예외 발생 유무를 파악하기 힘들기에,
