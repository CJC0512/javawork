...
public class Application3 {
    ...
    public static int solution(String input) throws IOException {
        ...
        /* 설명. 입력값이 공백 없이 이어서 들어옮에 따라 한 문자씩뜯어서 int 배열(map)에 옮겨담는 반복문 */
        for (int i = 0; i < N; i++) {
    ...
    private static void bfs(int x, int y) {
        ...
        q.add(new Node(x, y));           // q.offer도 가능

        while (!q.isEmpty()) {
            Node n = q.poll();

            /* 설명. 상하좌우를 눌러보는 개념의 for문(벽이 있는지, 지나온 길인지 확인 */
            for (int i = 0; i < 4; i++) {
                int curX = n.x + dirX[i];
                int curY = n.y + dirY[i];
                ...
                System.out.println("좌표: " + curX + ", " + curY);
