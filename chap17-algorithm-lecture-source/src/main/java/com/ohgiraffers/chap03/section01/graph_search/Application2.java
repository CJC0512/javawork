...
/* 수업목표. BFS와 x, y좌표를 활용한 문제를 해결할 수 있다. (feat. 배열의 인덱스와 반대 개념) */
/* 필기.
*   너비 우선 탐색(gBreadth-First Search)
*    선입선출 구조에 Queue를 활용한다.
*    시작 노드에서 출발해 시작 노드를 기준으로 가까운 노드를
*    먼저 방문하면서 탐색하는 알고리즘이다.*/
public class Application2 {
    ...
    public static int solution(String input) throws IOException {
        ...
        M = Integer.valueOf(st.nextToken());    // col
        N = Integer.valueOf(st.nextToken());    // row
        K = Integer.valueOf(st.nextToken());    // 배추 갯수
        ...
        for (int i = 0; i < K; i++) {
            ...
            /* 설명. 입력받은 x, y 좌표와 배열을 만들 떄의 순서(행, 열)는 반대의 개념이다. */
            map[y][x] = 1;
        }
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                ...
                /* 설명. 방문 안 한 위치에 배추가 심어져 있다면*/
                if (visit[i][j] == false && map[i][j] == 1) {
                    count++;
                    bfs(j, i);          // bfs의 인자 x좌표, y 좌표 순으로 받을 것이므로 i와 j를 확인하고 념겨준다.
                }
            }
        }
