...
public class JupiterAssertionsTests {
    ...
    /* 수업목표. junit5에서 제공하는 Assretions에서 제공하는 메소드에 대해 이해하고 활용할 수 있다. */

    /* 목차. 1. assertEquals*/
    /* 필기. assertEquals(expected, actual) 메소드는 실제 값과 기대 값의 일치 여부를 동일성으로 판단한다. */
    @Test
    void testAsssertEquals() {
        ...
        int result = cal.plusTwoNumbers(firstNum, secondNum);

        /* 설명. 3. then */
//        Assertions.assertEquals(expected, result);      // 단정문
//        Assertions.assertEquals(expected, result, 1);       // delta 오차범위 추가
